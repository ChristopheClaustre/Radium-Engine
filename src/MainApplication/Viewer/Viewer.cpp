...
namespace Ra
{
    ...
    void Gui::Viewer::initializeGL()
    {
        ...
        // FIXME(Mathias): width and height might be wrong the first time ResizeGL is called (see
        // QOpenGLWidget doc). This may cause problem on Retina display under MacOsX
        ...
        // NOTE(Charly) : Indeed, but on MSVC std::make_shared does not guarantee alignement, hence
        // making Eigen crash.
        //                We introduced Ra::Core::make_shared later and I still have to change all
        //                this calls.
    ...
    Gui::CameraInterface* Gui::Viewer::getCameraInterface() { return m_camera.get(); }

    Gui::GizmoManager* Gui::Viewer::getGizmoManager() { return m_gizmoManager; }

    const Engine::Renderer* Gui::Viewer::getRenderer() const { return m_currentRenderer; }
        ...
        switch (event->button())
        {
            case Qt::LeftButton: {
                ...
                if (event->modifiers().testFlag(Qt::ControlModifier))
                {
                    ...
                    auto macevent = new QMouseEvent(event->type(),
                                                    event->localPos(),
                                                    event->windowPos(),
                                                    event->screenPos(),
                                                    Qt::MiddleButton,
                ...
                if (isKeyPressed(Key_Space))
                {
                    ...
                    Core::Ray r = m_camera->getCamera()->getRayFromScreen(
                        Core::Vector2(event->x(), event->y()));
            ...
            case Qt::MiddleButton: { m_camera->handleMousePressEvent(event);
            }
            break;
            ...
            {
                // Check picking
                Engine::Renderer::PickingQuery query = {Core::Vector2(event->x(),
                                                                      height() - event->y()),
    ...
    void Gui::Viewer::grabFrame(const std::string& filename)
    {
        ...
        for (uint j = 0; j < tex->height(); ++j)
        {
            for (uint i = 0; i < tex->width(); ++i)
            {
                ...
                uint ou = 4 * ((tex->height() - 1 - j) * tex->width() + i); // Index in the final
                                                                            // image (note the j
                                                                            // flipping).

                writtenPixels[ou + 0] =
                    Ra::Core::Math::clamp<uchar>(pixels[in + 0] * 255.f, 0u, 0xffu);
                writtenPixels[ou + 1] =
                    Ra::Core::Math::clamp<uchar>(pixels[in + 1] * 255.f, 0u, 0xffu);
                writtenPixels[ou + 2] =
                    Ra::Core::Math::clamp<uchar>(pixels[in + 2] * 255.f, 0u, 0xffu);
    ...
    void Gui::Viewer::enableDebugDraw(int enabled) { m_currentRenderer->enableDebugDraw(enabled); }
} // namespace Ra
